...
void parse_args(int argc, char* argv[], options& opt)
{
	...
	{
		if (fileExists("firstplayer.txt"))
			remove("firstplayer.txt");
		if (fileExists("secondplayer.txt"))
			remove("secondplayer.txt");
		if (fileExists("gamestate.txt"))
			remove("gamestate.txt");
		if (fileExists("ships1.txt"))
			remove("ships1.txt");
		if (fileExists("ships2.txt"))
			remove("ships2.txt");

		opt.currentRound = 1;
		opt.rounds = atoi(argv[1]);
		opt.strategy1 = argv[2];

		if (argc == 3)
		{
			opt.nComputerPlayers = 1;
		}

		else if (argc == 4)
		{
			opt.nComputerPlayers = 2;
			opt.strategy2 = argv[3];
		}
	}
}
