...
void GameLogic::turn(Player* playerA, Player* playerB)
{
	pair<int, int> target;
	target = playerA->move();
	...
	bool wasSuccessful = playerB->checkShot(target);

	playerA->markShot(target, wasSuccessful);

	if (playerA->getSelected().getSize() > 1)
	{
		bool decision = playerA->decide();

		if (decision)
		{
			pair<int, int> target2;
			target2 = playerA->setTarget();

			playerA->getSelected().fire();
			bool wasSuccessful = playerB->checkShot(target2);
			playerA->markShot(target2, wasSuccessful);
		}
	}
}
