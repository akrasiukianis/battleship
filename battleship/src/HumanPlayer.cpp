...
void HumanPlayer::roundCounting(Ship* temp)
{
	if (temp->getSize() == 3)
	{
		...
		else if (temp->isReloading()) throw Ship::ShipHaveToWaitException();
		else if (ships[1]->isReloading()) ships[1]->add_roundCounter2(1);
	}
	else if (temp->getSize() == 2)
	{
		if (temp->get_roundCounter2() >= 1) temp->add_roundCounter2(0);
		else if (temp->isReloading()) throw Ship::ShipHaveToWaitException();
		else if (ships[2]->isReloading()) ships[2]->add_roundCounter3(1);
	}
	if (ships[1]->isReloading()) ships[1]->add_roundCounter2(1);
	if (ships[2]->isReloading()) ships[2]->add_roundCounter3(1);
